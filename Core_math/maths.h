...
//__device__ const float dPi = 3.141592653589f;

const float k2Pi = 2.0f*kPi;
//__device__  constexpr float d2Pi() { return 2.0f*dPi; }
const float kInvPi = 1.0f / kPi;
//__device__  constexpr float dInvPi() { return 1.0f / dPi; }
const float kInv2Pi = 0.5f / kPi;
//__device__ constexpr float dInv2Pi() { return 0.5f / dPi; }
const float kDegToRad = kPi / 180.0f;
//__device__  constexpr float dDegToRad() { return dPi / 180.0f; }
const float kRadToDeg = 180.0f / kPi;
//__device__  constexpr float dRadToDeg() { return 180.0f / dPi; }
...
inline float Dot(const Vector3& v, const Plane& p)
{
	return Dot(p, v);
}
...
//mostly taken from Real Time Collision Detection - p192
inline bool IntersectSegmentTri(const Vec3& p, const Vec3& q, const Vec3& a, const Vec3& b, const Vec3& c,  float& t, float& u, float& v, float& w, Vec3* normal, float expand)
{
	const Vec3 ab = b-a;
	const Vec3 ac = c-a;
	const Vec3 qp = p-q;
...
inline float ScalarTriple(const Vec3& a, const Vec3& b, const Vec3& c) { return Dot(Cross(a, b), c); }

// intersects a line (through points p and q, against a triangle a, b, c - mostly taken from Real Time Collision Detection - p186
inline bool IntersectLineTri(const Vec3& p, const Vec3& q, const Vec3& a, const Vec3& b, const Vec3& c)//,  float& t, float& u, float& v, float& w, Vec3* normal, float expand)
{
	const Vec3 pq = q-p;
	const Vec3 pa = a-p;
	const Vec3 pb = b-p;
...
// RTCD 5.1.5, page 142
inline  Vec3 ClosestPointOnTriangle(const Vec3& a, const Vec3& b, const Vec3& c, const Vec3& p, float& v, float& w)
{
	Vec3 ab = b-a;
	Vec3 ac = c-a;
	Vec3 ap = p-a;
...
inline float SqDistPointSegment(Vec3 a, Vec3 b, Vec3 c)
{
...
inline bool IntersectRayAABBOmpf(const Vec3& pos, const Vector3& rcp_dir, const Vector3& min, const Vector3& max, float& t) {
...
inline Vec3 rotate(const Vec3& q, float w, const Vec3& x)
{
	return 2.0f*(x*(w*w-0.5f) + Cross(q, x)*w + q*Dot(q, x));
}

// rotate x by inverse transform in (q, w)
inline Vec3 rotateInv(const Vec3& q, float w, const Vec3& x)
{
	return 2.0f*(x*(w*w-0.5f) - Cross(q, x)*w + q*Dot(q, x));
}

inline void TransformBounds(const Quat& q, Vec3 extents, Vec3& newExtents)
{
...
inline void TransformBounds(const Vec3& localLower, const Vec3& localUpper, const Vec3& translation, const Quat& rotation, float scale, Vec3& lower, Vec3& upper)
{
...
struct Bounds
{
	inline Bounds() : lower( FLT_MAX)
						   , upper(-FLT_MAX) {}

	inline Bounds(const Vec3& lower, const Vec3& upper) : lower(lower), upper(upper) {}

	inline Vec3 GetCenter() const { return 0.5f*(lower+upper); }
	inline Vec3 GetEdges() const { return upper-lower; }

	inline void Expand(float r)
	{
		lower -= Vec3(r);
		upper += Vec3(r);
	}
	...
	inline bool Empty() const { return lower.x >= upper.x || lower.y >= upper.y || lower.z >= upper.z; }

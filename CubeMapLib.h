...
namespace CML
{
    ...
    inline Vec3f SpherePoint2(int size, cCubeMapCoord c, float h)
    {


           //float  u_tangent_mix=1.00f;
          // float  u_tangent_angle= 3.14159265358979323846/4.0f;// 0.78539816339f;//3.1415 / 4.0;


            float vprex = c.mU+0.5;//+atan(c.mU * tan(u_tangent_angle));
            float vprey = c.mV+0.5;//+atan(c.mV * tan(u_tangent_angle));

            Vec3f v;

            switch(c.mFace)
            {
                case 0 :  v= Vec3f(  (size/2.0-vprex), -(size/2.0-vprey),  size/2.0f) * 2.0f/ size; //ok
                    break;
                case 1 :  v= Vec3f(  -(size/2.0-vprex),  -(size/2.0-vprey), -size/2.0f) * 2.0f/ size; //ok
                    break;
                case 2 :  v= Vec3f( -size/2.0f, -(size/2.0-vprex), (vprey-size/2.0)) * 2.0f/ size;  //need look
                    break;
                case 3 :  v= Vec3f(  size/2.0f,-(vprex-size/2.0), (vprey-size/2.0)) * 2.0f/ size; //ok
                    break;
                case 4 :  v= Vec3f( (size/2.0-vprey), size/2.0f, -(size/2.0-vprex)) * 2.0f/ size; //ok
                    break;
                case 5 :  v= Vec3f( -(vprey-size/2.0), -size/2.0f,(size/2.0-vprex)) * 2.0f/ size; //need look
                    break;

            //case 2 :  v= Vec3f( size/2.0f, (vprex-size/2.0),-(vprey-size/2.0)) * 2.0f/ size;
            //    break;
            }
            ...
            float z2 = v[2]* v[2];
            Vec3f s;
            s[0] = v[0] * sqrt(1.0f - y2 / 2.0f - z2 / 2.0f + y2 * z2 / 3.0f);
            s[1] = v[1] * sqrt(1.0f - x2 / 2.0f - z2 / 2.0f + x2 * z2 / 3.0f);
            s[2] = v[2] * sqrt(1.0f - x2 / 2.0f - y2 / 2.0f + x2 * y2 / 3.0f);
            return  s * h;
    }
}

...
#ifndef ENTRY_SDL_H_HEADER_GUARD
#define ENTRY_SDL_H_HEADER_GUARD
...
{
    Context()
        : m_width(2100)//(ENTRY_DEFAULT_WIDTH)
        , m_height(1500)//(ENTRY_DEFAULT_HEIGHT)
    ...
    int init_graph(int _argc, char** _argv)
    {
        m_mte.m_argc = _argc;
        m_mte.m_argv = _argv;
    ...
    int step_graph(Environment *env)
    {
        SDL_Event event;
        if (!m_exit)
        {
            mainTHUpdate(updateTextureFlag, tx_pointer, vbo_pointer, env);
            //bgfx::renderFrame();
    ...
    bool updateTextureFlag=false;
    uintptr_t tx_pointer[num_textures];
    uintptr_t vbo_pointer[num_textures];



};

inline static Context* s_ctx=new Context();

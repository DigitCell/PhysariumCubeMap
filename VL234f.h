...
enum AxisC { vl_x, vl_y, vl_z, vl_w };
typedef AxisC vl_axis;
...
inline float lerpvl(float a, float b, float s)
{ return((1.0f - s) * a + s * b); }
inline double lerpvl(double a, double b, double s)
{ return((1.0 - s) * a + s * b); }

template<class T_Value>
    inline T_Value lerpvl(T_Value x, T_Value y, Real s)
    { return(x + (y - x) * s); }

template<>
    inline float lerpvl<float>(float x, float y, Real s)
...
inline Vec2f& Vec2f::operator = (AxisC k)
{
    MakeUnit(k, vl_1);
...
inline Vec4f& Vec4f::operator = (AxisC k)
{
    MakeUnit(k, vl_1);
